...
### 2. 手机端配置 (包含 IP 与字体设置)
1. 用任意文本编辑器（如记事本、VSCode）打开 `anki遥控器.html` 文件。
2. **配置局域网 IP 地址** (必填)：
   找到代码中的 `ANKI_URL` 配置项，将其中的 `127.0.0.1` 修改为你运行 Anki 桌面端的电脑在局域网中的实际 IP 地址（例如 `192.168.1.100`）：
   ```javascript
   const ANKI_URL = 'http://192.168.1.100:8765';
   ```
3. **自定义手机端字体大小** (可选)：
   ...
   *(注：目前的默认大小是基于作者使用的 **小米 Max 3** 感到舒适的尺寸，其屏幕参数为：6.9 英寸 / 2160x1080 分辨率 / 350 ppi。如果你的手机屏幕比例或大小不同，请自行上下调整。)*
   ...
   ```
4. 将修改并保存后的 HTML 文件发送到手机端，或者在电脑上开启一个简易 HTTP 服务，在手机浏览器中输入地址打开。

### 3. 开始学习
保持电脑端 Anki 处于打开状态，手机端打开/刷新该 HTML 页面即可同步获取当前正在学习的卡片。
