...
- **智能防冲突与缓存绕过**：
  ...
  - **绕过 Anki 渲染缓存**：针对 AnkiConnect 刚修改 note 来不及重显的痛点，虚拟 DOM 动态灌入最新字段，确保实时同步刷新。
...
- **TTS 智能音量调控**：TTS 语音合成默认音量调低至 60%（0.6），完美匹配响度均衡后的卡片原音音量，并支持 0.0 ~ 1.0 的自由调节。
- **可视化配置与 PWA 全屏**：提供 `index.html` 通用网络版入口，支持手机端可视化填写 IP、调整字体、目标响度及 TTS 音量并永久记忆。完美支持添加到主屏幕化身为“免安装的全屏 App”。
...
3. 首次进入会弹出**可视化配置面板**。直接在输入框中填写你的 AnkiConnect 局域网 IP（如 `192.168.1.100:8765`，代码会自动补全 `http://`），并可随意调整适合你屏幕的各项字体大小、**目标响度 (Target LUFS)** 以及 **TTS 朗读音量 (默认 0.6)**。
...
### 4. 遥控器模式 (盲刷/不看屏幕)
如果你希望在不看屏幕下（例如把手机揣兜里戴耳机听卡片）进行盲刷复习，可以点击右上角 `⚙️` 并勾选 **开启遥控器模式**。
* **默认行为**：默认不开启，依旧使用手机屏幕上的九宫格点击逻辑。
* **支持的硬件设备**：本模式专门且明确强调支持的是 **弱智傻逼抖音遥控器** (此类廉价遥控器底层是通过模拟手指滑动和发送多媒体按键来工作的，而非发送标准键盘方向键)。具体蓝牙连接与故障排查可参考 👉 **[S2 抖音遥控器适配指南](./S2抖音遥控器适配指南.md)**。
* **遥控器按键映射规则**：
  * **⬆️ 上键 (向下滑动)**：正面时执行**翻面**；背面时调用手机本地 TTS 自动**朗读卡片 `.expression` 标签内的中文内容**。
  * **⬇️ 下键 (向上滑动) / 🆗 OK键 (原地点击)**：重播音频。
  * **⬅️ 左键 (向右滑动)**：正面时执行**翻面**；背面时评分 **Good (3分)**。
  * **➡️ 右键 (向左滑动)**：正面时执行**翻面**；背面时评分 **Again (1分)**。
  * **⏯️ 播放暂停键**：在任意状态下直接调用手机本地 TTS 自动**朗读卡片 `.expression` 标签内的中文内容**。

## 🎴 卡组模板参考

为了获得最佳的体验，建议配合特定的卡组模板使用。你可以参考我的示例模板进行配置：

👉 **[Anki 卡组模板示例](./卡组模板示例.md)**
